...
Transacties, Blokken, Mining, en de Blockchain

((("bitcoin","implementatie van", id="ix_ch02-asciidoc0", range="startofrange"))) Het bitcoin systeem, anders dan traditionele banken en betalingssystemen, is gebaseerd op gedecentraliseerd vertrouwen. In plaats van een centraal gereguleerde autoriteit, in bitcoin, vertrouwen wordt bereikt door een samenhang van de interacties van verscheidene deelnemers binnen het bitcoin systeem.In dit hoofdstuk, volgen we bitcoin op een hoog niveau, dit doen we door het volgen van één transactie door het bitcoin systeem en bekijken hoe het de status vertrouwd en geaccepteerd door het bitcoin mechanisme krijgt en waarna het uiteindelijk wordt geregistreerd in de blockchain, het gedistribueerde grootboek van alle transacties.

Ieder voorbeeld is gebaseerd op een actuele transactie welke is gemaakt binnen het bitcoin netwerk, simulerend de interacties tussen de gebruikers (Joe, Alice en Bob) door het verzenden van waarde van de ene portemonnee naar de andere. Wanneer we een transactie volgen door het bitcoin netwerk en de blockchain heen, gebruiken we de ((("blockchain explorer websites")))_blockchain explorer_ site om iedere stap te visualiseren. Een blockchain explorer is een web applicatie dat functioneert als een een bitcoin zoekmachine, die je in staat stelt om te zoeken op adressen, transacties, en blokken en  relaties zichtbaar maakt.

Populaire blockchain explorers zijn: ((("blockchain.info website")))((("blockexplorer.com")))((("blockr.io website")))((("insight.bitpay.com")))
...
Al deze hebben een zoekfunctie dat een adres, transactie hash, of block nummer en geeft alle relevante data aanwezig binnen het bitcoin netwerk en de blockchain weer. In ieder voorbeeld, geven we een URL weer dat je een directe link geeft naar de relevante data, zodat je deze zelf in detail kan bestuderen.


==== Bitcoin Overzicht

In het overzicht weergegeven in <<bitcoin-overview>>, zien we dat het bitcoin systeem bestaat uit gebruikers met een portemonnee met sleutels, transacties dat zich bewegen door het netwerk, en de miners welke (door competitiviteit) de blockchain vormen, wat het grootboek van alle transacties is. In dit hoofdstuk, volgen we een enkelvoudige transactie die zich door het netwerk begeeft en onderzoeken we de interactie tussen ieder onderdeel binnen het bitcoin systeem, op een hoog niveau. Opvolgende hoofdstukken geven een verdieping in de technologie achter de portemonnee, het minen, en de winkeliers systemen. 
...
Een kop koffie
==== Het kopen van een kop koffie

((("Transacties", id = "ix_ch02-asciidoc1", range = "startofrange"))) ((("transacties", "eenvoudig voorbeeld van", id = "ix_ch02-asciidoc2", range = "startofrange") )) Alice, geïntroduceerd in het vorige hoofdstuk, is een nieuwe gebruiker die net haar eerste bitcoin heeft verworven. In < <getting_first_bitcoin> >, Alice ontmoet haar vriend Joe om wat geld te wisselen voor Bitcoin. De transactie gecreëerd door Joe geeft Alice haar portemonnee een waarde van 0,10 BTC. Nu zal Alice haar eerste aankoop transactie maken, het kopen van een kopje koffie bij Bob's coffeeshop in Palo Alto, Californië. Bob's coffeeshop is onlangs begonnen met het accepteren van bitcoin betalingen, door een bitcoin optie aan zijn point-of-sale systeem toe te voegen. De prijzen bij Bob's Cafe zijn opgenomen in de lokale valuta (US dollars), maar bij de kassa, klanten hebben de mogelijkheid om te betalen in een van beide dollars of bitcoin. Alice plaatst haar bestelling voor een kopje koffie en Bob maakt een transactie bij de kassa. De point-of-sale systeem zal de de totale prijs van Amerikaanse dollars in bitcoins omzetten tegen de geldende marktrente en de prijzen worden weergeven in beide valuta's, een QR code bevat een _payment request_ voor deze transactie en wordt weergegeven (zie < <payment-request-QR> >):
...
[[payment-request-QR]]
.Betalingsverzoek QR code (Hint: Probeer deze te scannen!)
image::images/msbt_0202.png["payment-request"]

[[payment-request-URL]]
.de betalingsverzoek QR code bevat de volgende URL, gedefinieerd in BIP0021:
----
bitcoin:1GdK9UzpHBzqzX2A9JFP3Di4weBwqgmoQA?
bedrag=0.015&
van=Bob%27s%20Cafe&
omschrijving=Purchase%20at%20Bob%27s%20Cafe

Onderdelen van de URL 

Een bitcoin adres: "1GdK9UzpHBzqzX2A9JFP3Di4weBwqgmoQA"
Het betaalde bedrag: "0.015"
Een omschrijving voor het ontvangende adres: "Bob's Cafe"
Een omschrijving voor de betaling: "Aankoop bij Bob's Cafe"
----


[TIP]
====
((("QR-codes", "betalingsverzoeken als"))) In tegenstelling tot een QR code die het bestemmende bitcoin adres bevat , een betalingsverzoek is een QR-gecodeerde URL die een bestemmingsadres, een te betalen bedrag bevat, en een generieke omschrijving zoals "Bob's Cafe." Dit maakt het mogelijk voor een bitcoin portemonnee applicatie om deze informatie voorafgaand aan de betaling zo inzichtelijk te maken dit dit voor mensen te begrijpen is. U kunt de QR-code scannen met een bitcoin portemonnee applicatie om te zien wat Alice zou zien. 
====

Bob zegt: "Dat is een-dollar-vijftig, of vijftien millibits."

Alice gebruikt haar smartphone om de barcode op het scherm te scannen. Haar smartphone toont een betaling van +0.0150 BTC + op + Bob's Cafe + en ze selecteert + Verzenden + om de betaling te autoriseren. Binnen een paar seconden (ongeveer dezelfde hoeveelheid tijd als een credit card autorisatie), Bob ziet de transactie in het grootboek, en het voltooien van de transactie.

In de volgende paragrafen gaan we deze transactie in meer detail onderzoeken, zie hoe Alice's portemonnee is veranderd, hoe het werd verspreid over het netwerk, hoe het werd gecontroleerd, en ten slotte, hoe Bob dat bedrag kan besteden in de daaropvolgende transacties.

[NOTITIE]
====
De Bitcoin netwerk kan transacties verwerken in fractionele waarden, bijvoorbeeld uit milli-Bitcoins (1 / 1000ste van bitcoin) tot 1/100000000 van een bitcoin, die bekend is als ((("satoshis", "gedefinieerd")) ) satoshi. In dit boek zullen we de term "bitcoin" gebruiken om te verwijzen naar elke hoeveelheid bitcoin valuta, van de kleinste eenheid (1 Satoshi) tot het totale aantal (21.000.000) van alle bitcoins die ooit zal worden gemined. (((Range = "endofrange", startref = "ix_ch02-asciidoc2"))) 
====


=== Bitcoin Transacties

((("Transacties", "gedefinieerd"))) In eenvoudige termen, een transactie vertelt het netwerk dat de eigenaar van een aantal bitcoins de overdracht van een aantal van die bitcoins aan een andere eigenaar heeft toegekend. De nieuwe eigenaar kan nu deze bitcoins besteden door het creëren van een andere transactie die overbrenging toestaat aan een andere eigenaar, en zo verder, in een keten van eigendom. 
