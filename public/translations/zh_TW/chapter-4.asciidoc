...
一個比特幣的擁有權建立在數位鑰匙，比特幣地址及數位簽章之上。數位鑰匙實際上並不儲存在網路上，而是由使用者自己建立並儲存在自己的檔案/資料庫裡，儲存這些數位鑰匙的地方，稱為比特幣錢包。在比特幣錢包裡所有的數位鑰匙跟比特幣協定都沒有直接關係，而且可以在沒有blockchain幫助或是沒有網路的情況下產生及管理數位鑰匙。數位鑰匙帶來許多比特幣的有趣的特性，包括去中心化的信任和控制，所有權認證和加密型安全模型。 


每比比特幣交易要進到區塊鍊，都需要一組數位鑰匙來產生有效的簽章。因此，只要有這些數位鑰匙，就能控制這個比特幣帳號。一組數位鑰匙包括一把私鑰和一把公鑰。想想公鑰類似銀行帳號，私鑰則類似PIN碼或是支票上提供帳戶控制權的簽名。比特幣的用戶很少直接看到這些鑰匙。在大多數情況下，它們被存儲在錢包的文件內並且由比特幣錢包軟體進行管理。 

在比特幣交易的支付部分，收件人的公鑰是通過一稱為“比特幣地址”作為認證，類似於支票上受益人的名字。在大多數情況下，一把公鑰只能產生並對應一個比特幣地址。然而，並非所有比特幣地址都有其相對應的公鑰。比特幣地址也可以代表其他受益方式，比如說腳本(script)，我們將在本章後面看到。這樣一來，比特幣地址抽象化了資金的接受者，使得比特幣交易對象更靈活，類似於紙本支票：可用於支付進給其他人的賬戶，支付給公司賬戶，支付賬單，或者支付現金。比特幣地址是使用者最常看到能夠代表數位鑰匙的認證，這是使用者們需要與世界分享的部分。

在本章中，我們將介紹比特幣錢包，其中包含加密私鑰。我們將著眼於私鑰如何產生，存儲和管理。我們將回顧用於代表私鑰和公鑰，地址和腳本地址的各種編碼格式。最後，我們將看看鑰匙的特殊用途：對郵件簽名，證明所有權，並創造虛榮地址和紙錢包。 

==== 公鑰加密和虛擬貨幣

((("keys", id="ix_ch04-asciidoc0", range="startofrange")))((("cryptocurrency")))((("keys","cryptocurrency and")))((("keys","public")))((("public key cryptography")))((("public key cryptography","implementation of")))公共密鑰加密發明於1970年代，以數學為基楚的計算機和資訊安全。

在公共密鑰加密的發明中，發現了幾個合適的數學函數，如質數冪(prime number exponentiation)和橢圓曲線相乘(elliptic curve multiplication)。這些數學函數實際上是不可逆的，這意味著它們容易計算一個方向的結果但難以算出相反的方向的解。基於這些數學函數，密碼學可以建立數位秘密和不可偽造數位簽名。比特幣使用橢圓曲線乘法為基礎，作為它的公鑰加密。  
 
在比特幣中，我們用公鑰加密來建立鑰匙配對(key pair)，用來控制存取比特幣。鑰匙配對為一個私鑰和從這個私鑰產生出的一個唯一的公鑰的配對。公鑰用於接收比特幣，和私鑰用於簽署比特幣交易。

公鑰及私鑰之間的數學關係允許使用者用私鑰來產生訊息的簽章。此簽章可以用公鑰來驗證，並且不洩漏私鑰的資訊。

當花費比特幣的時候，目前比特幣持有者公開她的公鑰和簽名（每次都不同，但是從相同的私鑰創建）在交易中花費那些比特幣。通過公共密鑰和簽名的演示，大家在比特幣網絡可以驗證並接受該交易是有效的，確認傳輸比特幣的人在轉移的時候擁有這份比特幣的所有權。 

[TIP]
====
((("wallets","key pairs in")))為了方便，在大多數的比特幣錢包實作中，私鑰和公鑰以一組鑰匙配對為單位儲存在一起。然而，公鑰可以從私鑰計算得到，所以只存儲私鑰也是可能的。
====

[[private_public_keys]]
==== 私鑰和公鑰

((("keys","public/private")))((("keys","in wallets")))((("public keys")))((("wallets","public/private keys in")))一個比特幣錢包為許多鑰匙配對的集合，每個鑰匙配對代表一個私鑰和公鑰。私鑰（k）是一個數字，通常隨機選取。從私鑰，我們使用橢圓曲線乘法(elliptic curve multiplication)，單向密碼函數，以產生一個公鑰（K）。從公鑰（K），我們使用一個單向加密散列函數(hash)，以生成比特幣地址（A）。在本節中，我們將從產生私鑰開始，看看如何用橢圓曲線乘法轉成公鑰，最後再由此公鑰產生對應的比特幣地址。私鑰，公鑰和比特幣地址之間的關係示於< <k_to_K_to_A> >。
...
((("keys","private", id="ix_ch04-asciidoc1", range="startofrange")))((("private keys", id="ix_ch04-asciidoc2", range="startofrange")))私鑰是一個簡單的數字，隨機挑選。私鑰的所有權和控制權用於控制與此私鑰相對應的比特幣地址有關的所有資金。私鑰用來建立簽署，簽署是用來證明交易中所花費的必特幣資產的所有權。私鑰必須始終保持機密，因為它揭示給第三方，相當於讓他們在取得該私鑰所保護的比特幣的控制權。私鑰還必須進行備份，並防止意外丟失，因為如果失去了私鑰，將會無法回復並永遠遺失這些私鑰所擁有的資產。 

[TIP]
====
((("private keys","creating by hand")))比特幣私鑰只是一個數字。你可以用一枚硬幣，鉛筆和紙隨機挑選你的私鑰：擲硬幣256次，​​產生一組二進制數字，你就有一個隨機的比特幣私鑰放進比特幣錢包。然後公鑰可從此私鑰生成。
====

===== 生成一個來自隨機數的私鑰

((("private keys","generating from random numbers", id="ix_ch04-asciidoc3", range="startofrange")))生成私鑰的第一步，也是最重要的一步是要找到加密的安全來源，或隨機性。創建一個比特幣私鑰在本質上和“在1到2 ^ 256 ^之間挑一個數字”是一樣的。挑選數字的方法並不重要，只要它是不可預測的或重複的。比特幣軟件使用底層操作系統的隨機數產生器產生256位元的亂數（隨機性）。通常情況下，操作系統隨機數產生器由人隨機源，這就是為什麼你可能會被要求各地擺動你的鼠標，幾秒鐘初始化。對於真正的偏執狂，沒有什麼比得上骰子，鉛筆和紙。

更準確地說，私鑰可以是介在1到n-1之間的任何數，其中n是一個常數（n = 1.158 * 10 ^ 77 ^，略小於2 ^ 256 ）定義為順序在比特幣使用橢圓曲線（見<<elliptic_curve>>）。要建立一個這樣的鑰匙，我們隨機選擇一個256位的數字，並檢查它是否小於n-1。在程式語言的說法，這通常是通過將一個較大的隨機字串，從一個安全的隨機密碼來源，使用SHA256散列算法將可以方便地產生一個256位的數目。如果結果是小於n-1，我們就有一個合適的私鑰。否則，只要用另一個隨機數字再試一次。 
	
[TIP]
====
不要用自己寫的程式碼來產生一個隨機數或使用程式語言提供的“簡單”的隨機數產生器。使用((("密碼安全偽隨機數發生器(CSPRNG)")))加密安全偽隨機數發生器（CSPRNG）和具有足夠亂度的種子。詳讀你的隨機數產生器的文黨來確保他是有加密安全的。正確執行CSPRNG的是對私鑰的安全性是至關重要的。
====

以下是隨機生成的私有密鑰（k）以十六進制格式表示（用64位十六進制數字來表示256位二進制數字，每個數字能表示四個單位）：
...
[TIP]
====
比特幣私鑰的大小，2 ^ 256 是個不可思議的大小。這大約是10 ^ 77。可見宇宙估計含有10 ^ 80 個原子。
====

要用Bitcoin Core 客戶端來產生新的鑰匙(see <<ch03_bitcoin_client>>), 要用((("getnewaddress command (bitcoin-cli)"))) +getnewaddress+ 指令。安全性考量，他只會顯示公鑰，而非私鑰。 ((("dumpprivkey command (bitcoin-cli)")))((("private keys","exposing with bitcoind"))) 要讓它顯示出私鑰，用+dumpprivkey+ 指令。+dumpprivkey+ 指令會以稱作 _Wallet Import Format_ (WIF)的Base58 checksum-encoded形式，會在<<priv_formats>>有更多細節，來顯示私鑰。以下範例是用這兩個指令來顯示私鑰：
...
+dumpprivkey+指令會打開錢包並擷取出從 +getnewaddress+指令產生的私鑰。除了私鑰和公鑰都在皮包裡之外，不可能有其他方式知道公鑰對應的私鑰。 

[TIP]
=====================================================================
+dumpprivkey+ 這個指令並不是直接從公鑰產生出私鑰，因為這是不可能的。這條指令只是簡單的把之前呼叫getnewaddress產生的私鑰(比特幣錢包早就知道的私鑰)顯示出來。 
=====================================================================

你也可以透或Bitcoin 瀏覽指令工具(see <<libbitcoin>>) 來產生並顯示私鑰，透過以下指令。((("Bitcoin Explorer","seed command")))((("seed command (bx)"))) +seed+,((("Bitcoin Explorer","ec-new command")))((("ec-new command (bx)"))) +ec-new+ and((("Bitcoin Explorer","ec-to-wif command")))((("ec-to-wif command (bx)"))) +ec-to-wif+: (((range="endofrange", startref="ix_ch04-asciidoc3")))(((range="endofrange", startref="ix_ch04-asciidoc2")))(((range="endofrange", startref="ix_ch04-asciidoc1")))
...
((("keys","public")))((("public keys","generating")))公鑰是從私鑰透過橢圓曲線(不可逆)乘法得來。latexmath:[\(K = k * G\)]  _k_是私鑰，_G_是個叫做_generator point_的固定點，_K_是公鑰。“從_K_推得_k_”跟把每個+k+都是過一次一樣難。在我們演示如何從私鑰產生公鑰之前，讓我們先更深入的討論橢圓曲線加密。 


[[elliptic_curve]]
==== 橢圓曲線密碼解釋

((("elliptic curve cryptography", id="ix_ch04-asciidoc4", range="startofrange")))((("public key cryptography","elliptic curve cryptography", id="ix_ch04-asciidoc5", range="startofrange")))((("ECC", see="elliptic curve cryptography"))) 

<<ecc-curve>>是個橢圓曲線的例子，類似比特幣使用的橢圓曲線。

[[ecc-curve]]
一種橢圓曲線
image::images/msbt_0402.png["ecc-curve"]

比特幣使用由((("National Institute of Standards and Technology (NIST)")))National Institute of Standards and Technology(NIST)建立的((("secp256k1 curve standard"))) +secp256k1+標準。+secp256k1+的曲線由下列產生橢圓曲線的函示定義：
...
_mod p_ (對質數p取餘數) 指出這個曲線在一個有限的質數範圍_p_上， 也被寫作latexmath:[\(\mathbb{F}_p\)]，其中p = 2^256^ – 2^32^ – 2^9^ – 2^8^ – 2^7^ – 2^6^ – 2^4^ – 1，是一個非常大的質數。 

因為這條曲線是在一個有限的質數範圍上，而非實數範圍，所以線上的點的分佈就像是分散在二維平面中，讓這些點的分布很難被想像出來。但是他還是跟在實數範圍上的橢圓曲線有一樣的數學意義。以下舉<<ecc-over-F17-math>>為例，<<ecc-over-F17-math>>再以質數17為範圍的平面上表示出相同的橢圓曲線，並列出上面的點。+secp256k1+比特幣橢圓曲線有更複雜的點及不可思議的大的平面。 

[[ecc-over-F17-math]]
橢圓曲線加密：可視化上的橢圓曲線F(p)，其中p = 17
image::images/msbt_0403.png["ecc-over-F17-math"]

底下是+secp256k1+曲線上一點P(x,y)。可自行用Python檢查：
----
...
鍵入“help”，“copyright”，“credits”或“license”來取得詳細信息。
...
橢圓曲線中有一點稱為((("point at infinity (ECC)"))) "point at infinity"，像是在一般加法中的0的角色。電腦上常用 x = y = 0表示（雖然不滿足橢圓曲線等式，但這可用來簡單區別）。

還有((("+ operator")))((("elliptic curve cryptography","addition operator")))：[+]，被稱為”加法“，他就像國小學生學到的實數加法一樣。給定二點P~1~和P~2~在橢圓曲線上，還有第三點P~3~= P~1~+ P~2~，也在橢圓曲線上。

從幾何學來看，P~1~和P~2~之間畫一條線可得到P~3~。這條線恰會與橢圓曲線有一個交點。稱此點P~3~'=(X, Y)。然後，對X軸取對稱可得到P~3~=(X,-Y)。
