...
((("bitcoin network","transaction pools")))((("transaction pools")))((("memory pool")))((("mempool")))((("transactions","unconfirmed, pools of")))((("unconfirmed transactions")))比特幣網路上的節點幾乎都有維護一個未確認交易的暫時性列表，該列表稱為 _memory pool_， _mempool_ 或是 _transaction pool_。節點利用這個池來保持追蹤已廣播到網路但尚未被納入區塊鏈的交易。舉例來說，一個有用戶錢包的節點可以利用交易池來追蹤用戶錢包相對應的應收款項交易，這個應收款項交易已被廣播到網路但尚未得到確認納入區塊鏈。 

當交易被接收和驗證後，它們會被添加到交易池並中繼到相鄰節點來傳播到比特幣網絡上。

((("orphan transaction pool")))有些節點的實現版本有一個獨立的孤兒交易池。如未確認交易裡面的輸入交易還無法得知，例如缺少上游交易，該孤兒交易將被暫時存儲在孤兒交易池，直到上游交易到達。 
