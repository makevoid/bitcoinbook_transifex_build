...
=== Bitcoin Core: la implementació de referència
...
.Escollint un client bitcoin a bitcoin.org
image::images/msbt_0301.png["bitcoin choose client"]

==== Executant Bitcoin Core per primera vegada
...
.Pantalla de Bitcoin Core durant la inicialització de la cadena de blocs
...
==== Com obtenir informació sobre l'estat del client Bitcoin Core
...
==== Configuració del moneder i encriptació

Comandes: +encryptwallet+, +walletpassphrase+
...
==== Còpia de seguretat del moneder, volcat de text pla i restauració

Comandes: +backupwallet+, +importwallet+, +dumpwallet+
...
==== Adreces del moneder i rebuda de transaccions

Comandes: getnewaddress, getreceivedbyaddress, listtransactions, getaddressesbyaccount, getbalance
