...
((("blockchains", id = "ix_ch07-asciidoc0", rang = "startofrange"))) L'estructura de dades de la cadena de blocs és una llista ordenada amb enllaços apuntant enrera de blocs de transaccions. La cadena de blocs es pot emmagatzemar com un arxiu pla, o en una base de dades senzilla. El client Bitcoin Core emmagatzema les metadades de la cadena usant ((("base de dades LevelDB (Google)"))) la base de dades LevelDB de Google. Els blocs estan enllaçats "cap enrera", és a dir, cada bloc enllaça amb el bloc anterior de la cadena. La cadena de blocs sovint es visualitza com una pila vertical, amb els blocs situats un damunt de l'altre i el primer bloc que serveix de base de la pila. La visualització de blocs apilats uns sobre d'altres fa que es parli de termes com "altura" per referir-se a la distància des del primer bloc, i "cim" o "punta" per referir-se al bloc més recentment afegit. 
...
[[merkle_example]]
.Construcció d'un arbre de Merkle
====
[source, cpp]
----
...
[[block_structure2]]
.Eficiència de l'arbre de Merkle
[options="header"]
|=======
