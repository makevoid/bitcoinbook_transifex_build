...
== Claus, adreces, moneders

=== Introducció

((("bitcoin", "establir la propietat de"))) La propietat de bitcoin s'estableix a través _claus digitals_, _adreces bitcoin_ i _signatures digitals_. Les claus digitals no s'emmagatzemen realment a la xarxa, sinó que les creen i emmagatzemen els usuaris mateixos en un arxiu o base de dades senzilla, anomenada _moneder_. Les claus digitals del moneder d'un usuari són completament independents del protocol bitcoin i poden ser generades i gestionades pel programari del moneder de l'usuari sense connexió a la cadena de blocs o accés a Internet. Les claus permeten moltes de les propietats interessants de bitcoin, incloent la confiança i el control descentralitzats, certificació de propietat, i el model de seguretat de prova criptogràfica. 
...
==== Criptografia de clau pública i moneda digital
...
[[k_to_K_to_A]]
.Clau privada, clau pública, i adreça bitcoin
...
===== Generació d'una clau privada a partir d'un nombre aleatori
...
[[elliptic_curve]]
==== Descobrint la criptografia de corba el·líptica
...
Així, per exemple, el següent és un punt P de coordenades (x, y) que és un punt de la corba +secp256k1+. Pots comprovar-ho tu mateix utilitzant Python:
----
...
Hi ha un parell de casos especials que expliquen la necessitat del "punt en l'infinit."
...
La clau pública K es defineix com un punt +K = (x,y)+:
...
\end{equation}
++++
on K és la clau pública i A és l'adreça bitcoin resultant. 
...
[[pubkey_to_address]]
.Clau pública a adreça bitcoin: conversió d'una clau pública a una adreça bitcoin 
...
=== Implementació de claus i adreces amb Python  
