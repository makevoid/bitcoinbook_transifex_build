...
((("operacions", id = "ix_ch05-asciidoc0", rang =)) "startofrange") Les transaccions són la part més important del sistema bitcoin. Tota la resta a bitcoin està dissenyat per assegurar que les transaccions es poden crear, propagar per la xarxa, validar, i finalment afegir al llibre global de transaccions (la cadena de blocs). Les transaccions són estructures de dades que codifiquen la transferència de valor entre participants al sistema bitcoin. Cada transacció és una entrada pública a la cadena de blocs de bitcoin, el llibre comptable global de doble entrada. 

En aquest capítol examinarem totes les diverses formes de transaccions, el que contenen, com crear-les, com es verifiquen, i com arriben a formar part del registre permanent de totes les transaccions. 

[[tx_lifecycle]]
=== Cicle de vida de d'una transacció
...
[[tx_bcast]]
==== Difusió de transaccions a la xarxa bitcoin
...
[[tx_inputs_outputs]]
=== Sortides i entrades d'una transacció
...
Les sortides d'una transacció consten de dues parts: 
...
[[tx_out_structure]]
.L'estructura d'una sortida de transacció
[options="header"]
|=======
|Size| Field | Description
...
[[tx_in_structure]]
.L'estructura d'una entrada de transacció
[options="header"]
|=======
|Size| Field | Description
...
==== Afegit de taxes a les transaccions
...
[[tx_script]]
=== Scripts de transacció i llenguatge d'scripting
...
[[simplemath_script]]
.Script de validació de bitcoin amb operacions matemàtiques senzilles
...
Els dos scripts junts formarien el següent script de validació combinat:
