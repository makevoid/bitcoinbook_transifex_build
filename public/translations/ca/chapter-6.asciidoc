...
=== Arquitectura de xarxa punt a punt

((("La xarxa bitcoin", id = "ix_ch06-asciidoc0", rang = "startofrange"))) ((("La xarxa bitcoin", "arquitectura de"))) ((("xarxes punt a punt" ))) Bitcoin s'estructura com una arquitectura de xarxa punt a punt sobre Internet. El terme punt a punt o P2P, significa que els equips que participen a la xarxa es comuniquen entre sí, que són tots iguals, que no hi ha nodes "especials", i que tots els nodes comparteixen la responsabilitat de proporcionar serveis de xarxa. Els nodes de la xarxa estan connectats en una xarxa de malla amb una topologia "plana". No hi ha un servidor, no hi ha un servei centralitzat, i no hi ha una jerarquia dins de la xarxa. Tots els nodes en una xarxa punt a punt proporcionen i consumeixen serveis al mateix temps amb reciprocitat, i això actua com a incentiu per a la participació. Les xarxes punt a punt són inherentment resistents, descentralitzades i obertes. L'exemple per excel·lència d'una arquitectura de xarxa P2P va ser la mateixa xarxa inicial d'Internet, on els nodes de la xarxa IP eren iguals. L'arquitectura actual d'Internet és més jeràrquica, però el Protocol d'Internet encara conserva la seva essència de topologia plana. Més enllà de bitcoin, l'aplicació més gran i de més èxit dins les tecnologies P2P és el programari d'intercanvi d'arxius amb Napster com a pioner i BitTorrent com la més recent evolució de l'arquitectura.

L'arquitectura de la xarxa P2P de Bitcoin representa molt més que una elecció de topologia. Bitcoin és un sistema de pagament digital punt a punt per disseny, i l'arquitectura de la xarxa és alhora un reflex i els fonaments d'aquesta característica central. La descentralització del control és un principi de disseny bàsic i que només es pot aconseguir i mantenir a través d'una xarxa de consens P2P plana i descentralitzada. 

((("xarxa bitcoin", "definit"))) El terme "xarxa bitcoin" es refereix a la col·lecció de nodes que executen el protocol P2P de bitcoin. A més a més del protocol P2P bitcoin, hi ha altres protocols com ara ((("Stratum (STM) protocol de mineria"))) Stratum, que s'utilitzen per a la mineria i moneders lleugers o mòbils. Aquests protocols addicionals els proporcionen servidors d'enrutament de porta d'enllaç que accedeixen a la xarxa bitcoin utilitzant el protocol P2P de bitcoin, i que llavors estenen aquesta xarxa a nodes que executen altres protocols. Per exemple, els servidors Stratum connecten nodes de mineria Stratum a través del protocol Stratum a la xarxa bitcoin principal i enllacen el protocol Stratum amb el protocol P2P bitcoin. Utilitzem el terme "xarxa bitcoin estesa" per referir-nos a la xarxa global que inclou els protocols P2P bitcoin, protocols de mineria col·lectiva, el protocol Stratum, i d'altres protocols relacionats que connecten els components del sistema bitcoin. 

=== Tipus de nodes i funcions
...
[[full_node_reference]]
.Un node de xarxa bitcoin amb les quatre funcions: moneder, miner, base de dades completa de la cadena de blocs, i l'enrutament de xarxa
...
A més a més dels principals tipus de nodes al protocol P2P bitcoin, hi ha servidors i nodes que executen altres protocols, com ara protocols especialitzats de mineria col·lectiva i protocols lleugers d'accés de clients. 

<<node_type_ledgend>> shows the most common node types on the extended bitcoin network.

=== La xarxa de bitcoin estesa
...
[[node_type_ledgend]]
.Varis tipus de nodes de la xarxa estesa de bitcoin
image::images/msbt_0602.png["BitcoinNodeTypes"]

[[bitcoin_network]]
.La xarxa bitcoin estesa il·lustra els diversos tipus de nodes, portes d'enllaç i protocols
...
+PROTOCOL_VERSION+:: Una constant que defineix la versió del protocol P2P bitcoin que el client "parla" (per exemple, 70002)
+nLocalServices+ :: Una llista dels serveis locals suportats pel node, actualment només +NODE_NETWORK+
+nTime+:: L'hora actual
+addrYou+:: L'adreça IP del node remot vista des d'aquest node
+addrMe+:: L'adreça IP del node local, descoberta per el node local
+subver+:: Una versió que mostra el tipus de programari que s'està executant en aquest node (per exemple, "/Satoshi:0.9.2.1/")+
+BestHeight+:: L'alçada de bloc de la cadena de blocs d'aquest node

(Veure http://bit.ly/1qlsC7w[GitHub] per un exemple del missatge de xarxa +version+.)
...
[[network_handshake]]
.El handshake inicial entre les parts
...
[[address_propagation]]
.Propagació i descoberta de l'adreça
...
[[spv_nodes]]
=== Nodes de verificació de pagament simplificada (SPV)
...
=== Filtres bloom i actualitzacions d'inventari
...
Prioritat ::
Un nivell de prioritat d'alerta, actualment no s'utilitza
