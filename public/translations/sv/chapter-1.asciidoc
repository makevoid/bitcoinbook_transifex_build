...
Användare kan skicka bitcoin över nätverket för samma ändamål som vanliga valutor, däribland för att köpa och sälja varor, skicka pengar till personer och organisationer eller för att lagra värde. Bitcoin kan köpas, säljas och bytas mot andra valutor hos specialiserade valutaväxlare. På detta sätt kan Bitcoin anses vara perfekta pengar för internet, då det är snabbt, säkert och gränslöst.

Till skillnad från traditionella valutor så är bitcoin uteslutande virtuella. Det finns inga fysiska mynt eller ens digitala mynt. Istället är mynten antagna när en överföring av värde sker mellan sändare och mottagare. Användare av bitcoin innehar nycklar som tillåter dem att bevisa ägandeskap av transaktioner i nätverket vilket ger dem möjligheten att spendera värdet och överföra det till en ny mottagare. Dessa nycklar sparas oftast i en digital plånbok på varje användares dator. Innehav av dessa nycklar är det enda som krävs för att spendera bitcoin, vilket gör att all kontroll ligger hos dem enskilda användarna.

Bitcoin är ett distruberat icke-hierarkiskt system, och som sådant har det ingen central server eller kontroll. Bitcoins skapas genom en process som kallas "mining", i vilken deltagare tävlar om att hitta lösningar på matematisk problem samtidigt som dem bekräftar bitcoin transaktioner. Alla deltagare i bitcoin nätverket (dvs, alla som med en datorenhet kör bitcoins mjukvara) kan arbeta med mining genom att låta datorns kraft verifiera och lagra transaktioner. Var tionde minut, i genomsnitt, klarar någon deltagare av att bekräfta dem senaste tio minuternas transaktioner och belönas med nya bitcoin. I grund och botten så distribueras utgivandet av nya bitcoin och godkännandet av transaktioner som normalt sker hos en centralbank till samtliga deltagare runt om i världen i form av en tävling. 
...
1. Kan jag lita på att pengarna är giltiga och inte förfalskade?
2. Kan jag lita på att ingen annan kan hävda att pengarna tillhör dom och inte mig? (Ävan kallat ((("double-spend problem"))) "dubbelspendering" problemet)
...
* Ett distribuerat icke-hierarkiskt nätverk (bitcoin protokollet)
* En publik huvudbok (blockkjedjan)
* Ett distribuerat, matematiskt och förutsägbart valutautgivande (distribuerad mining)
* Ett distribuerat system för transaktionsbekräftelse (transaktionskod)
