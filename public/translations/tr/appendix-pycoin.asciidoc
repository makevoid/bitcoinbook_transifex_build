...
The Python library http://github.com/richardkiss/pycoin[+pycoin+], ilk olarak Richard Kiss tarafından kodlanmış ve denetlenen bitcoin anahtarlarının ve aktarımlarının değiştirilmesini destekleyen ve hatta standart olmayan işlemleri üreten betik dillerini destekleyen bir Python kütüphanesidir. 

pycoin kütüphanesi Python 2 (2.7.x) ve Python 3 (3.3 sonrasını) destekler, ve kolay kullanışlı ku ve tx gibi yararlı komut satırı seçenekleri ile birlikte gelir.
...
[UYARI]
====
Bu örnekteki parola kolayca tahmin edilecek kadar basittir
====
----
$ ku P:foo
...
bilgiyi JSON formatında almak için:
...
Birden fazla alt anahtar üretmek:
...
Karşılık gelen adreslerin üretilmesi:
...
Açık anahtar çiftinden (Test ağında)
