...
((("blockchains", id="ix_ch07-asciidoc0", range="startofrange")))Blok zinciri veri yapısı sıralı, geriye bağlı işlem bloklarının listesidir. Blok zinciri düz bir dosya olarak veya basit bir veri tabanında saklanabilir. Bitcoin Çekirdeği blok zinciri metadatasını ((("LevelDB database (Google)"))) Google'ın LevelDB adlı veri tabanı ile saklar. Bloklar "geriye" bağlıdır, her biri zincirdeki bir önceki bloğa atıfta bulunur. Blok zinciri genelde ilk blok yığının temeli olacak şekilde üst üste dikilmiş bloklar halinde dikey bir yığın olarak görselleştirilir. Üst üste yığılmış blokların görselleştirilmesinde kullanılan "yükseklik" terimi ilk bloğa olan uzaklığı ve "üst" veya "uç" terimleri de zincire eklenmiş en son blokları belirtir. 

Blok zincirindeki her bir blok o bloğun başlık kısmında SHA256 algoritması kullanan kriptografik özet fonksiyonu ile oluşturulmuş kriptografik özeti ile tanımlanır.  Her blok ayrıca ((("ebeveyn bloklar")))_parent_ olarak bilinen bir önceki bloğun kriptografik özeti ile bağlıdır. Bir başka deyişle, her blok kendi ebeveyninin özet değerini başlık kısmında barındırır. Bu her biri ebeveynine bağlanmış kriptografik özetler zinciri ((("yaratılış bloğu"))) _genesis block_ olarak bilinen ilk oluşturulmuş bloğa kadar devam eder.  

"Yarım Kaldı Düzenle" Bir bloğun sadece bir ebeveyni olmasına rağmen geçici olarak birden fazla çocuğu olabilir, Her bir çocuk diğer bloklar gibi başlık kısımlarındaki "önceli blok özeti" alanında aynı ebeveyn bloğun kriptografik özetini barındırır.  Multiple children arise during a blockchain "fork," a temporary situation that occurs when different blocks are discovered almost simultaneously by different miners (see <<forks>>). Eventually, only one child block becomes part of the blockchain and the "fork" is resolved. Even though a block may have more than one child, each block can have only one parent. This is because a block has one single "previous block hash" field referencing its single parent. 
...
|=======
|Boyut| Alan | Tanım
| 4 bayt | Blok Uzunluğu| Bu alandan sonra gelen bloğun bayt cinsinden uzunluğu
| 80 bayt | Blok Başlığı | Blok başlığını oluşturan alanlar.
| 1-9 bayt (VarInt) | İşlem Sayacı | Kaç işlem var
| Değişken | İşlemler | Bu blokta kayıtlı işlemler
|=======
...
|=======
|Boyut| Alan | Tanım
| 4 bayt | Sürüm | Yazılım/ protokol güncellemelerini takip etmek için sürüm numarası
| 32 bayt | Önceki Bloğun Özei | Zincirde bir önce gelen (baba) bloğun özeti
| 32 bayt | Merkle Kökü | Bu bloktaki işlemlerin merkle ağacı kökünün özeti
| 4 bayt | Zaman damgası| Bu bloğun yaklaşık yaratılma zamanı (Unix Epoch'dan beri geçen saniye)
| 4 bayt | Zorluk Hedefi | Bu blok için İş-kanıtı algoritmasının zorluk hedefi
| 4 bayt | Nons | İş kanıtı algoritmasında kullanılan bir sayaç
|=======
...
=== Blok Zincirinde Blokları Bağlamak
...
----
{
 ...
 #[... daha birçok işleme yer verilmemiştir ...]
...
[[simple_merkle]]
.Merkle ağacı düğümlerini hesaplamak
...
[[merkle_tree_large]]
.Birçok veri elemanını özetleyen bir Merkle ağacı
...
[[merkle_example_run]]
.Örnek merkle kodunun derlenmesi ve çalıştırılması
====
[source,bash]
----
...
=== Merkle Ağaçları ve Basitleştirilmiş Ödeme Doğrulaması (BÖD)
