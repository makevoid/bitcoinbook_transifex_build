...
Thư viện Python http://github.com/richardkiss/pycoin[+pycoin+], ban đầu được viết và bảo trì bởi Richard Kiss. Đây là thư viện Python hỗ trợ thao tác trên các giao dịch và khoá bitcoin, thậm chí còn hỗ trợ ngôn ngữ lập trình kịch bản đủ để xử lý chính xác các giao dịch không thuộc nhóm chuẩn. 

Thư viện pycoin hỗ trợ cả Python 2 (2.7.x) và Python 3 (after 3.3), và đi kèm với một số tiện ích dòng lệnh hữu ích, ku và tx.
