[[ch03_bitcoin_client]]
== Trình khách (client) Bitcoin

== Bitcoin Core: Cài đặt tham khảo

((("bitcoin client", id="ix_ch03-asciidoc0", range="startofrange")))((("bitcoin client","Bitcoin Core", id="ix_ch03-asciidoc1", range="startofrange")))((("Bitcoin Core client", id="ix_ch03-asciidoc2", range="startofrange")))((("Satoshi client", see="Bitcoin Core client")))Bạn có thể tải về chương trình khách tham khảo _Bitcoin Core_, còn được gọi là "Satoshi client," từ bitcoin.org. Trình khách tham khảo cài đặt tất cả các khía cạnh của hệ thống bitcoin, bao gồm ví, bộ phận kiểm tra giao dịch và bản sao đầy đủ của sổ cái giao dịch (blockchain), và một nút mạng đầy đủ trên mạng bitcoin ngang hàng.

Trên((("bitcoin.org","Bitcoin Core, downloading"))) http://bitcoin.org/en/choose-your-wallet[Bitcoin's Choose Your Wallet page], chọn Bitcoin Core để tải về trình khác tham khảo. Tuỳ thuộc vào hệ điều hành của bạn, bạn sẽ tải về một chương trình cài đặt. Đối với Windows, đó là một tệp tin nén dạng ZIP hay dạng chạy được .exe. Đối với Mac OS đó là một tệp ảnh đĩa .dmg. Các phiên bản dành cho Linux gồm có gói PPA cho Ubuntu hay tệp tin nén tar.gz. Trang bitcoin.org liệt kê các trình khách bitcoin được khuyên dùng có thể xem ở <<bitcoin-choose-client>>.

[[bitcoin-choose-client]]
.Choosing a bitcoin client at bitcoin.org
image::images/msbt_0301.png["bitcoin choose client"]

==== Chạy Bitcoin Core lần đầu
