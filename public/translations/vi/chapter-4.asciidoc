...
((("bitcoin","establishing ownership of")))Quyền sở hữu với bitcoin được thiết lập thông qua _các khoá số_, _các địa chỉ bitcoin_, và _các chữ ký số_. Các khóa số không thực sự được lưu trên mạng, nhưng thay vào đó chúng nó được tạo ra và lưu trữ bởi người dùng vào một tệp tin hoặc một cơ sở dữ liệu đơn giản, có tên gọi là _ví_. Các khoá số trong ví của một người dùng hoàn toàn độc lập với giao thức bitcoin và có thể được sinh ra và quản lý bởi phần mềm ví của người dùng mà không cần sự hiện diện của chuỗi khối hay kết nối vào mạng internet. Các khóa làm nên nhiều thuộc tính thú vị trong bitcoin, bao gồm cơ chế kiểm soát và uỷ thác phi tập trung, chứng nhận quyền sở hữu, và mô hình bảo mật được bảo vệ bằng mật mã. 
...
==== Mật mã khóa công khai và Đồng tiền mật mã
