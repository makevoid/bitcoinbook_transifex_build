...
=== Giao dịch, Khối, Đào mỏ và Chuỗi khối

((("bitcoin","implementation of", id="ix_ch02-asciidoc0", range="startofrange")))Hệ thống bitcoin, không giống như ngân hàng và các hệ thống thanh toán truyền thống, vì nó dựa trên sự uỷ thác phân tán. Thay vì phải có một cơ quan uỷ thác trung tâm, thì ở bitcoin, việc uỷ thác lại là một đặc tính nổi trội xuất hiện từ việc tương tác giữa các thành viên khác nhau trong hệ thống bitcoin. Trong chương này, chúng ta sẽ xem xét bitcoin từ góc độ tổng quan bằng việc bám sát một giao dịch đi qua hệ thống bitcoin và xem xét xem khi nào nó được uỷ thác và được chấp nhận bởi cơ chế đồng thuận phân hoá (distributed consensus) và cuối cùng được ghi vào chuỗi khối, tức sổ cái phân hoá chứa tất cả các giao dịch.
