...
((("bitcoin", id="ix_ch01-asciidoc0", range="startofrange")))((("bitcoin","defined")))比特币系统（Bitcoin）是基于一系列概念和技术集合所形成的数字货币生态系统。这种货币基于比特币（bitcoin）单位，可用于在比特币网络上存储和传输价值。根本上，比特币用户通过互联网，使用比特币协议彼此交流。这个比特币协议栈（注：栈，计算机术语中指实现应用依赖的底层技术），通过开源协议放出，能广泛地在不同计算设备上运行，包括便携式电脑和智能手机，使这种技术很容易接入。

通过比特币交易，用户可以在网络上来做任何传统货币可以做的事情： 买卖商品， 汇款给个人或组织，以及展延信用。 比特币具有快捷，安全和无界的特点。所以从某种角度来说，比特币是互联网货币的完美形态。

与传统货币不同，比特币是完全虚拟的。比特币不是实体硬币，本质上它连数字硬币都不是。它仅在交易过程中存在意义，用来表明交易过程中出资人给收款人的交易值。比特币用户都各自拥有自己的密钥，这些密钥可以使每个用户，在比特币网络中，都掌握自己交易的主动权。通过这些密钥，比特币用户可以解锁自己的比特币数值，并将这些比特币汇款给另一个比特币用户。这些密钥通常被用户保存在给自电脑中的数字钱包里。使用密钥解锁自己的交易是支付比特币的必要条件，因此，用户可以完全的掌控自己的交易。
