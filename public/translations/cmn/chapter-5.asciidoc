...
((("交易", id="ix_ch05-asciidoc0", range="startofrange")))交易是比特币系统中最重要的部分。比特币的一切设计都是用来保证交易可以被正确地创建、传播、确认并最终加入到全局交易账簿——“区块链”中。比特币系统参与者间的价值转移都以特定格式保存在交易中。每一笔交易都被公开在比特币区块链——一个全局的分布式记账单中。 

在本章中，我们来了解一下交易有哪些不同形式、包括了哪些部分，它们是如何被创建、验证，并被永久地添加到交易记录中。 

[[tx_lifecycle]]
=== 交易的生命周期

((("transactions","lifecycle of", id="ix_ch05-asciidoc1", range="startofrange")))交易的生命周期始于交易的创建，或者说是 _生成_。随后一个或多个签名被添加到交易中，来授予这个交易使用资金余额的权力，接着交易被广播到比特币网络，网络上的每一个节点（参与者）验证和传播，直到网络上（几乎）所有节点都接收到这个交易。最后，挖矿节点确认这个交易，把它打包进一个包含有很多交易的区块，并记录到区块链中。 

一旦被记录到区块链并被足够多的后续区块确认，这个交易就永久地成为比特币账簿中的一部分，并被全网参与者认可。通过这个交易，其中的资金被分配给一个新的拥有者，这个拥有者可以在新的交易中动用这笔资金，扩展所有权链并开始一个新的交易生命周期。 
