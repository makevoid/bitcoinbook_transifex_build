...
((("bitcoin", id="ix_ch01-asciidoc0", range="startofrange")))((("bitcoin","defined")))Bitcoin ist eine Sammlung von Konzepten und Technologien die die Basis für ein digitales Finanz-Ökosystem bilden. Währungseinheiten gennant Bitcoins werden hierzu verwendet um Wert zu speichern und zwischen den Teilnehmern des Bitcoin Netzwerks zu übertragen. Bitcoin Nutzer kommunizieren mit Hilfe des Bitcoin Protokolls in erste Linie über das Internet, obwohl andere Transportnetze auch verwendet werden können. Der Bitcoin Protokoll-Stack, verfügbar als Open Source Software, kann auf einem breiten Spektrum an Geräten, inklusive Laptops und Smartphones, ausgeführt werden und ist damit eine leicht zugängliche Technologie.

Benutzer können Bitcoins über das Netzwerk transferieren und damit dasselbe tun wie mit konventionellen Währungen, Kaufen und Verkaufen von Waren, Überweisungen an Personen oder Unternehmen durchführen oder Geld verleihen. Bitcoins können gekauft, verkauft und gegen andere Währungen an spezialisierten Börsen gehandelt werden. Bitcoin ist eigentlich die perfekte Art von Geld für das Internet weil es schnell, sicher und grenzenlos ist.

Anders als traditionelle Währungen sind Bitcoins ausschließlich virtuell. Da gibt es auch keine physischen Münzen oder auch digitale Münzen per se. Die Münzen sind in den Transaktionen impliziert die den Wert vom Sendenden zum Empfänger transferieren. Benutzer von Bitcoin besitzen Schlüssel die es ihnen erlauben das Eigentum an Transaktionen im Bitcoin-Netzwerk nachzuweisen und den Wert freischalten, verwenden und an einen neuen Empfänger übertragen zu können. Diese Schlüssel sind oft in digitalen Brieftaschen auf den Computern der Benutzer gespeichert. Der Besitz eines Schlüssels der eine Transaktion freischalten kann ist die einzige Voraussetzung um Bitcoins ausgeben zu können - das legt die Kontrolle gänzlich in die Hände eines jeden Benutzers.

Bitcoin ist ein verteiltes, Peer-to-Peer-System. Als solches ist da kein "zentraler" Server oder Kontrollinstanz. Bitcoins werden durch einen Prozess genannt "Mining" (Schürfen) kreiert, dieser Prozess beinhaltet das konkurrierende Lösen von mathematischen Problemen und dem gleichzeitigen verarbeiten von Bitcoin Transaktionen. Jeder Teilnehmer am Bitcoin-Netzwerk (z.B., jeder der ein Gerät mit ein mit einer vollständigen Bitcoin Anwendung betreibt) kann als Miner fungieren und durch die Rechenkapazität des Computers Transaktionen verifizieren und eintragen. Im Durchschnitt alle 10 Minuten ist jemand in der Lage die Transaktionen der letzten 10 Minuten zu validieren und wird mit nagelneuen Bitcoins belohnt. Im Grunde dezentralisiert das Bitcoin-Mining die Geldmengenerweiterungs- und -verringerungsfunktion einer Zentralbank und ersetzt die Notwendigkeit aller Zentralbanken mit dieser globalen Konkurrenzkampf.  
 
((("mining","algorithms regulating"))) Das Bitcoin Protokoll besitzt eingebaute Algorithmen die das Bitcoin-Mining über das Netzwerk regulieren. Der Schwierigkeitsgrad für die Verarbeitung durch Miner – um erfolgreich einen Block von Transaktionen für das Bitcoin-Netzwerk zu speichern – wird dynamisch angepasst, damit im Durchschnitt, alle 10 Minuten ein Miner erfolg hat. Egal wie viele Miner (und CPUs) zu jeder Zeit an der Aufgabe arbeiten. ((("bitcoin","rate of issuance")))Das Bitcoin-Protokoll halbiert auch die Rate mit der neue Bitcoins generiert werden alle 4 Jahre und limitiert die Gesamtmenge an Bitcoins die generiert werden können zu insgesamt 21 Millionen Bitcoins. Das führt dazu das die Anzahl an Bitcoins im Umlauf einer leicht vorhersehbaren Kurve folgen die 21 Millionen im Jahr 2140 erreichen wird. Über lange Sicht ist Bitcoin als Währung deflationär - aufgrund der abnehmenden Rate mit der Bitcoins ausgeschüttet werden. Weiters können Bitcoins nicht inflationiert werden indem Geld über die erwartete Ausschüttungsrate hinaus "gedruckt" werden. 

Hinter den Kulissen ist Bitcoin auch der Name des Protokolls, des Netzwerkes und einer Innovation bei dezentralen Computersystemen. Die Bitcoin-Währung ist in Wirklichkeit nur die erste Anwendung dieser Erfindung. Als Entwickler sehe ich Bitcoin als das Internet des Geldes, ein Netzwerk um Werte zu übertragen und um das Eigentum von digitalen Vermögen mittels verteilter Rechenleistung zu sichern. Da steckt viel mehr hinter Bitcoin als auf den ersten Blick zu sehen ist.  

In diesem Kapitel werden wir beginnen die Hauptkonzepte und Begriffe zu erklären, die nötige Software herunterladen und Bitcoin zu nutzen um einfache Transaktionen durchzuführen. In den folgenden Kapiteln werden wir beginnen die einzelnen Schichten der Technologie, die Bitcoin ermöglicht, zu zerlegen und die internen Funktionen des Bitcoin-Netzwerks und Protokolls zu untersuchen.  

.Digitale Währungen vor Bitcoin
****

((("bitcoin","precursors to"))) Die Entstehung von nachhaltigem digitalem Geld ist eng verbunden mit den Entwicklungen in der Kryptographie. Dies ist nicht erstaunlich, wenn man die fundamentalen Herausforderungen anschaut bei der Nutzung von bits als Representation von Wert beim Austausch von Gütern und Dienstleistungen. Zwei universell wichtige Fragen bei der Aktzeptanz von digitalem Geld sind: 

1. Kann ich darauf vertrauen, dass das Geld authentisch und nicht gefälscht ist?
2. Kann ich sicher sein, dass niemand sonst dieses Geld beanspruchen kann? (Aka the((("double-spend problem"))) “double-spend” problem.)
...
[[user-stories]]
=== Bitcoin Verwendung, Benutzer und ihre Geschichten
