...
Nesse capítulo, nós iremos primeiramente examinar a mineração como um mecanismo de oferta monetária e então olharemos para a mais importante função da mineração: o mecanismo de consenso emergergente descentralizado subjacente da segurança do bitcoin.

==== A Economia do Bitcoin e a Criação de Moedas

((("currency creation", id="ix_ch08-asciidoc2", range="startofrange")))((("mining","currency creation", id="ix_ch08-asciidoc3", range="startofrange")))Os bitcoins são minerados durante a criação de cada bloco em uma taxa fixa e que só diminui. Cada bloco, gerado a cada 10 minutos em média, contém bitcoins completamente novos, que foram criados do nada. A cada 210.000 blocos, ou aproximadamente a cada 4 anos, a taxa de emissão da moeda é diminuída em 50%. Nos primeiros 4 anos de operação da rede, cada bloco continha 50 bitcoins novos. 

Em novembro de 2012, a taxa de emissão de bitcoins foi diminuída para 25 bitcoins/bloco, e ela irá reduzir novamente para 12,5 bitcoins/bloco no bloco 420.000, que só será minerado aproximadamente em 2016. A taxa de novas moedas diminui exponencialmente em 64 "divisões por dois" até o bloco 13.230.000 (que será minerado aproximadamente no ano 2137), quando ela atingirá a unidade mínima de 1 satoshi. Finalmente, após 13,44 milhões de blocos, aproximadamente no ano de 2140, quase 2,099,999,997,690,000, ou quase 21 milhões de bitcoins, terão sido emitidos. Depois disso, os blocos não irão conter novos bitcoins, e os mineradores serão recompensados apenas com as taxas de transações. <<bitcoin_money_supply>> demonstra o número total de bitcoins em circulação ao longo do tempo, na medida que a emissão da moeda diminui.

[[bitcoin_money_supply]]
.Oferta de moedas bitcoin ao longo do tempo baseado em uma taxa de emissão com decréscimo geométrico
image::images/msbt_0801.png["BitcoinMoneySupply"]

[NOTE]
====
O número máximo de "coins" mineradas é o _limite superior_ das recompensas de mineração possíveis do bitcoin. Na prática, um minerador pode intencionalmente minerar um bloco recebendo menos do que a recompensa completa. Tais blocos já foram minerados e mais podem ser minerados no futuro, resultando em uma menor emissão total de moedas.
====

No código de exemplo em <<max_money>>, nós calculamos a quantidade total de bitcoins que será emitida.
...
((("inflation, resisting")))A emissão decrescente e finita cria uma oferta monetária fixa que resiste à inflação. Ao contrário das moedas fiduciárias, que podem ser impressas em números infitos por um banco central, o bitcoin jamais sofrerá inflação causada por impressão de moedas.

.Dinheiro deflacionário
****
((("deflationary money")))A consequência mais importante e debatida de uma emissão monetária fixa e descrescente é que a moeda tenderá a ser inerentemente _deflacionária_. A deflação é o fenômeno da perda de valor devido a um defasamento entre a oferta e a demanda que aumenta o valor (e a taxa de câmbio) de uma moeda. A deflação dos preços, o contrário da inflação, significa que o dinheiro tem mais poder de compra com o passar do tempo.

Muitos economistas argumentam que uma economia deflacionária é um desastre e deveria ser evitada a todo custo. Isso porque em um período de rápida deflação, as pessoas tendem a guardar dinheiro ao invés de gastá-lo, na esperança de que os preços irão cair. Tal fenômeno ocorrer durante a "Década Perdida" do Japão, quando um colapso completo da demanda levou a moeda para um espiral deflacionário. 
...
((("consensus","decentralized")))((("decentralized consensus")))No capítulo anterior, nós aprendemos sobre a blockchain, o registro público (lista) global de todas as transações, que todos participantes da rede bitcoin aceitam como um registro oficial das posses dos bitcoins. 
...
* Verificação independente de cada transação, por todos nodos completos, baseado em uma extensa lista de critérios
...
----
Alta Prioridade > 100.000.000 satoshis * 144 blocos / 250 bytes = 57.600.000
----
...
[[table_8-1]]
.A estrutura de um input de transação "normal"
[options="header"]
|=======
...
[[table_8-2]]
.A estrutura da geração de um input de transação
[options="header"]
|=======
|Tamanho| Campo | Descrição
...
((("block headers","constructing")))Para construir o cabeçalho do bloco, o nodo minerador precisa preenchê-lo em seis campos, como listado em <<block_header_structure_ch08>>.
...
2009:: 0,5 MH/s–8 MH/s (crescimento de 16&#x00D7; )
2010:: 8 MH/s–116 GH/s (crescimento de 14.500&#x00D7;)
2011:: 16 GH/s–9 TH/s (crescimento de 562&#x00D7;)
2012:: 9 TH/s–23 TH/s (crescimento de 2.5&#x00D7;)
2013:: 23 TH/s–10 PH/s (crescimento de 450&#x00D7;)
2014:: 10 PH/s–150 PH/s em Agosto (crescimento de 15&#x00D7;)
...
.Poder de hashing total, em gigahashes por segundo, ao longo de dois anos
...
[[bitcoin_difficulty]]
.Métrica de dificuldade de mineração do Bitcoin, ao longo de dois anos
