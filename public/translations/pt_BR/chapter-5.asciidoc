...
((("transactions","creating")))Pode ser útil pensar em uma transação da mesma maneira que um cheque de papel. Como um cheque, a transação é um instrumento que expressa a intenção de se transferir dinheiro e ela não é visível ao sistema financeiro até que seja enviada para execução/depósito. Como um cheque, quem origina a transaçõa não tem a obrigação de ser aquele que assina a transação. 
...
Assim que a transação for criada, ela é assinada pelo dono (ou donos) dos fundos de origem. Ela é propriamente formada e assinada, a transação assinada agora é válida e contém toda a informação necessária para executar a transferência dos fundos. Finalmente, a transação válida tem que atingir a rede bitcoin para que ela possa ser propagada até atingir um minerador para incluí-la no registro público (a blockchain).
...
((("bitcoin network","propagating transactions on")))((("transactions","propagating")))Assim que a transação é enviada para qualquer nodo conectado à rede bitcoin, a transação será validada por aquele nodo. Se for válida, o nodo irá propagá-la para outros nodos com os quais ele está conectado, e uma mensagem de sucesso irá retornar na mesma hora para quem originou a transação. Se a transação for inválida, o nodo irá rejeitá-la e retornará na mesma hora uma mensagem de rejeição para quem originou a transação. 
...
[TIP]
====
((("accounts")))((("balances")))Não há contas correntes ou saldos no bitcoin; existem apenas _outputs de transações não-gastas_ (UTXO) espalhadas na blockchain. 
====
...
A exceção para a cadeia de output e input é um tipo especial de transação chamada transação _coinbase_, que é a primeira transação em cada bloco. Essa transação é inserida no bloco pelo minerador "vencedor" e cria novos bitcoins que serão pagos para o minerador como uma recompensa por ter conseguido minerar o bloco. É assim que a oferta monetária do bitcoin é criada durante o processo de mineração, como veremos no <<ch8>>.
...
* Uma quantidade de bitcoin, denominados em _satoshis_, a menor unidade bitcoin
...
[[tx_out_structure]]
.A estrutura de um output de transação
[options="header"]
|=======
|Tamanho| Campo | Descrição
...
[[get_utxo]]
.Um script que chama o API da blockchain.info para encontrar o UTXO relacionado a um endereço
====
[source, python]
----
...
[[get_utxo_run]]
.Executando o script get-utxo.py
====
[source,bash]
----
$ python get-utxo.py 
...
[[select_utxo]]
.Um script para calcular quantos bitcoins totais serão emitidos
====
[source, python]
----
...
[[select_utxo_run]]
.Executando o script select-utxo.py
====
----
...
[[tx_in_structure]]
.A estrutura de um input de transação
[options="header"]
|=======
|Tamanho| Campo | Descrição
...
O algoritmo atualmente utilizado pelos mineradores para priorizar a inclusão no bloco das transações de acordo com suas taxas é examinado em detalhes no <<ch8>>.(((range="endofrange", startref="ix_ch05-asciidoc6")))
    
==== Adicionando Taxas às Transações

((("fees, transaction","adding", id="ix_ch05-asciidoc7", range="startofrange")))((("transactions","fees", id="ix_ch05-asciidoc8", range="startofrange")))A estrutura de dados das transações não tem um campo para taxas. Ao invés disso, infere-se que as taxas são a diferença entre a soma dos inputs e a soma dos outputs. Qualquer valor em excesso que permanece após todos os outputs terem sido deduzidos de todos os inputs é a taxa que é coletada pelos mineradores. 
...
Agora vamos ver um cenário diferente. Eugênia, nossa diretora do centro de caridade para crianças nas Filipinas, completou uma campanha para arrecadação de fundos para comprar livros escolares para as crianças. Ela recebeu milhares de pequenas doações de pessoas de todos os lugares do mundo, totalizando 50 bitcoins, então sua carteira está cheia de pagamentos muito pequenos (UTXO). Agora ela que comprar centenas de livros escolares da editor local, pagando em bitcoin. 
...
((("scripts","construction of")))((("validation (transaction)","script construction for")))O mecanismo de validação da transação do Bitcoin depende em dois tipos de scripts para validar as transações: um script de travamento e um de destravamento. 
...
.Combinando scriptSig e scriptPubKey para avaliar um script de transação
