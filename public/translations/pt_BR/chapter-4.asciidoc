...
((("bitcoin","estabelecendo a posse do")))A posse do bitcoin é estabelecida através de _chaves digitais_, _endereços bitcoin_ e _assinaturas digitais_. Na realidade, as chaves digitais não são armazenadas na rede, mas ao invés disso, são criadas e armazenadas pelo usuário em um arquivo, um banco de dados simples, chamado de _wallet_ (carteira). As chaves digitais em uma carteira de um usuário são completamente independentes do protocolo bitcoin e podem ser geradas e gerenciadas pelo software de carteira do usuário sem qualquer referência à blockchain ou acesso à internet. As chaves permitem muitas das interessantes propriedades do bitcoin, incluindo confiança descentralizada e controle, atestação de posse e o modelo de segurança por prova criptográfica. 
...
Neste capítulo iremos apresentar Carteiras, as quais contém Chaves Criptográficas. Nós iremos aprender como chaves são geradas, armazenadas e gerenciadas. Iremos revisar os vários formatos de codificação usados para representar chaves privadas e públicas, endereços e endereços baseados em script. Finalmente iremos considerar usos especiais para chaves: para assinar mensagens, comprovar propriedade, criar endereços exclusivos e carteiras em papel. 

==== Criptografia da Chave Pública e Criptomoeda

((("keys", id="ix_ch04-asciidoc0", range="startofrange")))((("cryptocurrency")))((("keys","cryptocurrency and")))((("keys","public")))((("public key cryptography")))((("public key cryptography","implementation of")))A criptografia de chave pública foi inventada nos anos 1970s e é uma fundação matemática para a segurança da computação e informação.

Desde a invenção da criptografia de chave pública, muitas funções matemáticas adequadas foram descobertas, como ((("prime number exponentiation")))exponenciação de números primos e multiplicação de curva elíptica. Essas funções matemáticas são praticamente irreversíveis, significando que elas são fáceis de calcular em uma direção, e inviáveis de serem calculadas na direção oposta. Baseada nessas funções matemáticas, a criptografia permite a criação de segredos digitais e assinaturas digitais que não podem ser esquecidas. O bitcoin usa multiplicação de curva elíptica como base para sua criptografia de chave pública.  
...
[TIP]
====
((("wallets","key pairs in")))Na maioria das implementações de carteira, as chaves privadas e públicas são armazenadas juntas como um _par de chaves_, por conveniência. No entanto, a chave pública pode ser calculada a partir da chave privada, então também é possível se armazenar apenas a chave privada.
====

[[private_public_keys]]
==== Chaves Privada e Pública

((("keys","public/private")))((("keys","in wallets")))((("public keys")))((("wallets","public/private keys in")))Uma carteira bitcoin contém um grupo de pares de chaves, cada um consistindo de uma chave privada e uma chave pública. A chave privada (k) é um número, geralmente escolhido ao acaso. A partir da chave privada, nós usamos multiplicação em curva elíptica, uma função criptográfica de um único sentido, para gerar a chave pública (K). A partir da chave pública (K), nós iremos usar uma função hash criptográfica de um sentido para gerar um endereço bitcoin (A). Nessa seção, nós iniciaremos com a geração da chave privada, analisaremos a matemática de curva elíptica que é usada para torná-la em uma chave pública e, finalmente, a geração do endereço bitcoin a partir da chave pública. A relação entre a chave privada, chave pública e endereço bitcoin está demonstrada em <<k_to_K_to_A>>.

[[k_to_K_to_A]]
.Chave privada, chave pública e endereço bitcoin
image::images/msbt_0401.png["privk_to_pubK_to_addressA"]

[[private_keys]]
==== Chaves Privadas

((("keys","private", id="ix_ch04-asciidoc1", range="startofrange")))((("private keys", id="ix_ch04-asciidoc2", range="startofrange")))Uma chave privada é simplesmente um número, escolhido ao acaso. A posse e o controle sobre a chave privada é a fonte do controle do usuário sobre todos os fundos associados ao endereço bitcoin correspondente. A chave privada é usada para criar assinaturas que são necessárias para se gastar bitcoins ao comprovar a posse dos fundos usados em uma transação. A chave privada deve sempre ser mantida em segredo, porque revelá-la a terceiros é equivalente a dar a eles controle sobre todos os bitcoins protegidos por aquela chave. Também deve ser feito backup da chave privada, além de protegê-la de perdas acidentais, porque a sua perda não pode ser recuperada e os fundos protegidos por ela também são perdidos para sempre. 

[TIP]
====
((("private keys","creating by hand")))A chave privada bitcoin é apenas um número. Você pode escolher suas chaves privadas aleatoriamente usando uma moeda, um lápis e um papel: jogue a moeda 256 vezes e você terá os digitos binários de uma chave privada aleatória que você pode usar em uma carteira bitcoin. A chave pública pode então ser gerada a partir de sua chave privada.
====
...
Abaixo está demonstrada uma chave privada (k) gerada aleatoriamente mostrada em formato hexadecial (256 dígitos binários mostrados como 64 dígitos hexadecimais, cada um com 4 bits):
...
[TIP]
====
O tamanho do espaço possível de Chaves Privadas existentes, 2^256 é de tamanho imcompreensível. É aproximadamente 10^77 na escala decimal. O universo visível estima conter 10^80 átomos.
====
...
((("keys","public")))((("public keys","generating")))A chave pública é calculada a partir da chave privada usando uma multiplicação de curva elíptica, que é irreversível: latexmath:[\(K = k * G\)] onde _k_ é a chave privada, _G_ é um ponto constante chamado de _ponto gerador_ e _K_ é a chave pública resultante. A operação inversa, conhecida como "encontrando o logaritmo discreto"—calculando _k_ se você sabe o __K__—é tão difícil quanto tentar todos os possíveis valores de +k+, ou seja, uma busca de força bruta. Antes de nós demonstrarmos como gerar uma chave pública a partir de uma chave privada, vamos analisar a criptografia de curva elíptica em um pouco mais de detalhes. 


[[elliptic_curve]]
==== Explicando a Criptografia de Curva Elíptica

((("elliptic curve cryptography", id="ix_ch04-asciidoc4", range="startofrange")))((("public key cryptography","elliptic curve cryptography", id="ix_ch04-asciidoc5", range="startofrange")))((("ECC", see="elliptic curve cryptography")))A criptografia de curva elíptica é um tipo decriptografia de chave pública ou assimétrica baseada em um problema logarítimico discreto que é expressado pela adição e multiplicação nos pontos de uma curva elíptica. 

<<ecc-curve>> é um exemplo de uma curva elítpica, similar à usada pelo bitcoin.
...
Existem vários casos especiais que explicam a necessidade deum "ponto no infinito."
...
[TIP]
====
((("addresses, bitcoin","public keys vs.")))((("public keys","bitcoin addresses vs.")))Um endereço bitcoin _não_ é a mesma coisa que uma chave pública. Endereços bitcoin são derivados a partir de uma chave pública utilizando-se uma função de sentido único. 
====
...
[[pubkey_to_address]]
.Chave pública para endereço bitcoin: conversão de uma chave pública em um endereço bitcoin 
...
[[addr_example]]
.Criando um endereço bitcoin codificado em Base58Check a partir de uma chave privada
====
[source, cpp]
----
...
[[addr_example_run]]
.Compilando e executando o addr code
====
[source,bash]
----
...
((("private keys","format")))((("Bitcoin Explorer","modifying private key formats with")))A chave privada pode ser representada em vários formatos diferentes, os quais todos correspondem ao mesmo número de 256 bits. A <<table_4-2>> demonstra três formatos comumente usados para representar chaves privadas.

[[table_4-2]]
.Representações de chave priva (formatos de codificação)
[options="header"]
|=======
...
<<table_4-3>> demonstra a chave privada nesses três formatos.
...
[[table_4-3]]
.Exemplo: Mesma chave, diferentes formatos
[options="header"]
|=======
...
((("public keys","formats of")))As chaves públicas também são apresentadas de diferentes maneiras, principalmente como chaves públicas _comprimidas_ ou _não-comprimidas_. 
...
Aqui está a chave pública gerada pela chave privada que nós criamos anteriormente, mostrada como as coordenadas +x+ e +y+:
...
<<table_4-4>> mostra a mesma chave, codificadanos formatos WIF e WIF-comprimido.
...
| Hex | 1E99423A4ED27608A15A2616A2B0E9E52CED330AC530EDCC32C8FFC6A526AEDD
| WIF | 5J3mBbAH58CpQ3Y5RNJpUKPE62SQ5tfcvU2JpbnkeyhfsYB1Jcn
| Hex-comprimido | 1E99423A4ED27608A15A2616A2B0E9E52CED330AC530EDCC32C8FFC6A526AEDD_01_
| WIF-comprimida | KxFC1jmwwCoACiCAWZ3eXa96mBM6tb3TYzGmf6YwgdGWZgawvrtJ
|=======
...
.Geração de chaves e endereços e formatação com a livraria pybitcointools
====
[source,python]
----
...
<<key-to-address_script_run>> mostra o output após executar esse código.
...
O endereço bitcoin (b58check) é: 
 1thMirt546nngXqyPEz532S8fLwbozud8
O endereço bitcoin comprimido (b58check) é: 
...
<<ec_math>> ((("elliptic curve cryptography","in Python")))((("Python ECDSA library"))) é outro exemplo, usando a livraria Python ECDSA para a matemática da curva elítptica e sem usar quaisquer livrarias bitcoin especializadas.

[[ec_math]]
.Um script demonstrado a matemática da curva elíptica usada para as chaves bitcoin
====
[source, python]
----
...
[[ec_math_run]]
.Instalando a livraria Python ECDSA e executando o script ec_math.py
====
----
...
((("wallets", id="ix_ch04-asciidoc23", range="startofrange")))As carteiras são recipientes para chaves privadas, geralmente implementadas como arquivos estruturas ou bancos de dados simples. 
...
[[table_4-10]]
.Exemplo de chave privada criptograda em BIP0038 
|=======
...
[[paper_wallet_simple]]
.Um exemplo de uma carteira de papel simples originada em bitaddress.org
