...
((("Сделки", ID = "ix_ch05-asciidoc0", диапазон = "startofrange"))) Транзакции являются наиболее важной частью системы Bitcoin. Все остальное в Bitcoin предназначено для обеспечения нормальных операций по созданию, распространению по сети, проверки, и, наконец, добавления к глобальной книге сделок (blockchain). Транзакции -- это структуры данных, которые кодируют передачу ценностей между участниками системы. Каждая транзакция соответствует записи в глобальной бухгалтерской книге под названием blockchain. 
