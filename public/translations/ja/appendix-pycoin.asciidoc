...
Pythonライブラリ http://github.com/richardkiss/pycoin[+pycoin+] はBitcoinキーやトランザクションの操作をサポートしているPythonベースライブラリです。元々Richard Kissによって書かれメンテナンスされているライブラリです。このライブラリは非標準トランザクションを適切に扱うことができるscript言語さえもサポートしています。 

このpycoinライブラリはPython 2 (2.7.x)とPython 3 (after 3.3)を両方ともサポートしており、使いやすいコマンドラインツールkuとtxが付属しています。

=== キーユーティリティー(KU)

((("key utility (ku)", id="ix_appdx-pycoin-asciidoc0", range="startofrange"))) コマンドラインユーティリティ +ku+ ("キーユーティリティ")は、キーを操作するためのスイス・アーミーナイフです。それはBIP32キー、WIF、およびアドレス（Bitcoinとaltcoin）をサポートしています。以下はいくつかの例です。

GPGと _/dev/random_ のデフォルトのエントロピー源を用いBIP32キーを作成:
...
[WARNING]
====
この例で使われているパスフレーズはとても予想しやすいものを使っています。
====
----
$ ku P:foo
...
Dogecoinアドレスとして作成:(((range="endofrange", startref="ix_appdx-pycoin-asciidoc0")))
...
((("transaction utility (tx)")))コマンドラインユーティリティ +tx+ は、人間が読める形でのトランザクション表示、pycoinのトランザクションキャッシュまたはウェブサービス(現在blockchain.infoとblockr.io、biteasy.comに対応)からのベーストランザクション取得、トランザクションのマージ、インプットまたはアウトプットの追加削除、トランザクションへの署名、ができます。
...
アッと！ウェブサービスの設定をしていませんでした。それを今やりましょう。
====
[source,bash]
----
...
これらの設定は自動的に行われていません。コマンドラインツールが、あなたがどのトランザクションに興味を持っているかという個人情報をサードパーティのウェブサイトに漏洩しないようにするためです。もし気にしないのであれば、 _.profile_ にこれらの行を入れておき毎回設定しなくてもよいようにできます。
...
最後の行のメッセージは、トランザクションの署名を検証するために現れています。このため、コマンドラインオプションとして +-a+ を追加してトランザクションの署名検証を行ってみましょう:
...
ここで、特定アドレスに対する未使用トランザクションアウトプットを見てみましょう。ブロック番号１には +12c6DSiU4Rq3P4ZxziKxzrL5LmMBrzjrJX+ へのcoinbaseトランザクションが見えます。 +fetch_unspent+ を使ってこのアドレスにある全てのbitcoinを見てみましょう:
